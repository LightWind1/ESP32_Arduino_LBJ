...
static void SpiEnd(void)
{
  ...
  SPI.endTransaction();
  SPI.end();
  digitalWrite(CC1101_SCK_PIN, LOW);
}
/****************************************************************
*FUNCTION NAME:Reset
...
static void HwSPI_Init(void)
{
  ...
  digitalWrite(CC1101_CS_PIN, HIGH);
  digitalWrite(CC1101_SCK_PIN, HIGH);
  digitalWrite(CC1101_CS_PIN, LOW);
  Reset();                    //CC1101 reset
  SpiEnd();
}
...
static int8_t SPIsetMaskedRegValue(uint8_t reg, uint8_t value, uint8_t msb, uint8_t lsb)
{
	...
	printf("[CC1101]SPI set masked value failed!\r\n");
	printf("REG=%02Xh,PreviousValue=%02Xh,Mask=%02Xh,\r\n"
...
{	
	...
	//pinMode(CC1101_GDO2_PIN, INPUT_PULLDOWN);
...
/*
	...
	*/
	if(digitalRead(CC1101_GDO2_PIN))			//receive data
	{
		while (digitalRead(CC1101_GDO2_PIN));
		return 1;
	}
	else							// no data
	{
...
*@retval	错误码
-----------------------------------------------------------------------*/
int8_t CC1101_SetPacketLengthMode(uint8_t mode, uint8_t len)//0x00 16
{
	uint8_t len_limit = 0;
	int8_t state;
...
*@retval	无
-----------------------------------------------------------------------*/
int8_t CC1101_Transmit(uint8_t* data, uint32_t len)
{
					 ...
					 int8_t power, uint8_t preambleLength)
{
	...
	while((i < 10) && !flagFound)
	{
		...
		Serial.print("CC1101_REG_VERSION = ");
	...
	if(!flagFound)
	{
		Serial.println("No CC1101 found!");
		return(RADIO_ERR_CHIP_NOT_FOUND);
	}
	else
		Serial.println("Found CC1101!");

	int8_t state = 0;
	
	//0.先设置为IDLE态，后直接访问寄存器进行多种初始化设置，尤其是API无法完成的配置设置(重点!)
	state = MiscRegisterConfigs();
	RADIO_ASSERT(state);
	//1.设置射频工作频率
	...
	//uint8_t version = SPIreadRegister(CC1101_REG_ADDR);
	//Serial.println(version,HEX);
	return(state);
}
